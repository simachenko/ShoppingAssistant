...
/// injected into the chat history as an exact id/name list before every turn, so the language
/// model can resolve an ordinal/descriptive reference to a known id rather than guessing or
/// re-deriving it from prior prose. Actually resolving natural language to a position is the
/// LLM's job and isn't asserted here (no live LLM in this test project) — only that the context
/// it needs is present.
/// </summary>
public class FollowUpQuestionTests
{
    ...
    public async Task A_session_with_prior_search_results_injects_their_ids_into_the_chat_history()
    {
        ...
        var chatClient = new FakeChatClient("Sure, here's more about the Galaxy S24.");
